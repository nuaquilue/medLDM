...
fire.regime = function(land, coord, orography, clim, pfst.pwind, all.swc, clim.sever, t, 
  ...
  for(swc in all.swc){
    ...
    if(swc<4){ 
      ## Fixed
      if(sum(climatic.severity[climatic.severity$year==t,2:4])>0){  
        is.aba.fix = T
        area.target = min(200000-(annual.aburnt+annual.asupp), climatic.severity[climatic.severity$year==t, swc+1])
      }
      ## Find stochastic annual burnt area
      else{ 
        is.aba.fix = F
        if(clim.sever==1 & swc==1){  # decide if climatic severity is extrem for 'wind' or 'heat' swc
          pctg = hot.days[hot.days$year==t, swc+1]
        ...
        area.target = round(min(200000-(annual.aburnt+annual.asupp), 
                             max(10,rlnorm(1, burnt.area.dist$meanlog[burnt.area.dist$clim==clim.sever & burnt.area.dist$swc==swc],
                                              burnt.area.dist$sdlog[burnt.area.dist$clim==clim.sever & burnt.area.dist$swc==swc])))) 
    ...
    while(area.target>0){
      ...
      while((aburnt.lowintens+aburnt.highintens+asupp.sprd+asupp.fuel)<fire.size.target){
        ...
        if(clim.sever==0){
          aburnt.lowintens = aburnt.lowintens + sum(sprd.rate$burn  & !sprd.rate$tosupp.sprd & !sprd.rate$tosupp.fuel & sprd.rate$fi<=ifelse(swc<4,params$fire.intens.th,100))
          aburnt.highintens = aburnt.highintens + sum(sprd.rate$burn & !sprd.rate$tosupp.sprd & !sprd.rate$tosupp.fuel & sprd.rate$fi>ifelse(swc<4,params$fire.intens.th,100))
        }
        else{
